...
namespace CPF
{
    ...
    public class UIElement : Visual, IComponent//, IEnumerable<UIElement>//, INamed//, IInputElement
    {
        ...
        internal void RaiseDeviceEvent(EventArgs e, EventType eventName)
        {
            ...
            else if (eventName == EventType.PreviewMouseDown)
            {
                OnPreviewMouseDown((MouseButtonEventArgs)e);
            }
            else if (eventName == EventType.PreviewMouseUp)
            {
                OnPreviewMouseUp((MouseButtonEventArgs)e);
            }
            else if (eventName == EventType.MouseDown)
            {
                ...
                if (lastMouseDownTime != 0)
                {
                    ...
                    if (Platform.Application.GetRuntimePlatform().DoubleClickTime > (now - time))
                    {
            ...
            else if (eventName == EventType.MouseEnter)
            {
                OnMouseEnter((MouseEventArgs)e);
            }
            else if (eventName == EventType.MouseLeave)
            {
                OnMouseLeave((MouseEventArgs)e);
            }
            else if (eventName == EventType.MouseWheel)
            {
                OnMouseWheel((MouseWheelEventArgs)e);
            }
            else if (eventName == EventType.KeyDown)
            {
                OnKeyDown((KeyEventArgs)e);
            }
            else if (eventName == EventType.KeyUp)
            {
                OnKeyUp((KeyEventArgs)e);
            }
            else if (eventName == EventType.TextInput)
            {
                OnTextInput((TextInputEventArgs)e);
                //}
                //else if (eventName == EventType.DoubleClick)
                //{
                //    if (lastMouseDownTime != 0)
                //    {
                //        var time = DateTime.FromBinary(lastMouseDownTime);
            ...
            else if (eventName == EventType.DragOver)
            {
                OnDragOver((DragEventArgs)e);
            }
            else if (eventName == EventType.DragEnter)
            {
                OnDragEnter((DragEventArgs)e);
            }
            else if (eventName == EventType.DragLeave)
            {
                OnDragLeave(e);
            }
            else if (eventName == EventType.Drop)
            {
        ...
        public virtual string Name
        {
            get
            {
                ...
                //{
                //    return ((IComponent)this).Site.Name;
                //}
                return GetValue<string>();
            }
            set
            {
                //if (!string.IsNullOrEmpty(value) && ((IComponent)this).Site != null)
                //{
                //    ((IComponent)this).Site.Name = value;
                //}
                SetValue(value);
            }
        }
        ...
        {
            get { return (NavigationMethod?)GetValue(); }
            private set { SetValue(value); }
        }
        ...
        {
            get { return GetValue<PointField>(); }
            set { SetValue(value); }
        }
        /// <summary>
        ...
        protected override void OnPropertyChanged(string propertyName, object oldValue, object newValue, PropertyMetadataAttribute propertyMetadata)
        {
            ...
            if (data != null)
            {
                ...
                else if (data.AffectsArrange)
                {
                    InvalidateArrange();
                    //Parent?.OnChildInvalidateArrange(this);
                }
                else if (data.AffectsRender)
                {
                    Invalidate();
                }
            ...
            if (!element.HasLocalOrStyleValue(propertyName, out var p))
            {
                ...
                if (p != null)
                {
                    if (p is UIPropertyMetadataAttribute ui && ui.Inherits)
                    {
                        ...
                        element.inheritsValues.Add(propertyName, new InheritsValue { Value = newValue, ValueForm = ValueForm.Property });
                    }
        ...
        internal override void OnSetValue(string propertyName, PropertyMetadataAttribute property, ValueForm valueForm, object newValue)
        {
        ...
        {
            if (inheritsValues.TryGetValue(propertyName, out InheritsValue value1) && value1.ValueForm == ValueForm.Property)
            {
                inheritsValues.Remove(propertyName);
            }
        }
        internal override void OnClearTriggerValue(string propertyName, EffectiveValue value)
        {
            ...
            {
                if (inheritsValues.TryGetValue(propertyName, out InheritsValue value1) && value1.ValueForm == ValueForm.Trigger)
                {
                    inheritsValues.Remove(propertyName);
                }
            }
        ...
        internal override void OnClearAnimationValue(string propertyName, EffectiveValue value)
        {
            ...
            {
                if (inheritsValues.TryGetValue(propertyName, out InheritsValue value1) && value1.ValueForm == ValueForm.Animation)
                {
                    inheritsValues.Remove(propertyName);
                }
            }
        ...
        internal override void OnClearStyleValue(string propertyName, EffectiveValue value)
        {
            ...
            {
                if (inheritsValues.TryGetValue(propertyName, out InheritsValue value1) && value1.ValueForm == ValueForm.Style)
                {
                    inheritsValues.Remove(propertyName);
                }
            }
                            ...
                            if (p.inheritsPropertyName != null)
                            {
                                if (p.inheritsPropertyName.Contains(item))
                                {
                                    ...
                                    else if (p.HasLocalOrStyleValue(item, out _))
                                    {
                                        ...
                                        var old = GetValue(item);
                                        inheritsValues.Remove(item);
                                        v = new InheritsValue { Value = value, ValueForm = ValueForm.Property };
                                        inheritsValues.Add(item, v);
                                        if (!old.Equal(v.Value))
                                        {
                                            //OnPropertyChanged(item, old, v.Value, GetPropertyMetadata(item));
        ...
        //public static readonly RoutedEvent MouseLeaveEvent = new RoutedEvent("MouseLeave", UIElementType, MouseEventType);
        ...
        //public static readonly RoutedEvent MouseWheelEvent = new RoutedEvent("MouseWheel", UIElementType, MouseEventType);
        ...
        //public static Type KeyEventType = typeof(EventHandler<KeyEventArgs>);
        //public static readonly RoutedEvent KeyDownEvent = new RoutedEvent("KeyDown", UIElementType, KeyEventType);
