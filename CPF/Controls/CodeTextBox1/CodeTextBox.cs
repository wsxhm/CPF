...
namespace CPF.Controls
{
    ...
    [DefaultProperty(nameof(CodeTextBox.Text))]
    public class CodeTextBox : Control, IDocumentStyle, IEditor
    {
        public CodeTextBox()
        {
        ...
        {
            return new TextBoxViewEx(this)
            {
                Name = "TextBoxViewEx",
                PresenterFor = this,
                Cursor = Cursors.Ibeam,
                //Width = (this, nameof(WordWarp), a => (bool)a ? (FloatField)"100%" : (FloatField)"auto"),
                Bindings = {
                    /*{ nameof(TextBoxViewEx.XoffsetDraw), nameof(UIElement.Width),FindPresenterByName<Panel>("line1"),
                        BindingMode.OneWay, (data) => {
                            if (data is FloatField) {
                                return ((FloatField)data).Value;
                            }
                            return 0;
                        }
                    },*/
                    {
                        nameof(Width),nameof(WordWarp),this,BindingMode.OneWay,(bool data)=>{
                            return data ? (FloatField)"100%" : (FloatField)"auto";
                        }
                    },
                    {
                        nameof(LineNumWidth),nameof(LineNumWidth),this
                    }
                },
            };
        }
        ...
        /// 内部TextBoxView
        /// </summary>
        protected ITextBoxView TextBoxView
        {
            get
            {
                return view;
        ...
        private void Children_CollectionChanged(object sender, CollectionChangedEventArgs<IDocumentElement> e)
        {
            if (textChanged)
            {
                ...
                {
                    //计算行号长度
                    var a = document.Lines.Count;
                    LineNumWidth = Math.Abs(a).ToString().Length;
        ...
        {
            List<(DocumentChar dchar, int index)> token = new List<(DocumentChar dchar, int index)>();
            var index = 0;
            foreach (DocumentChar item_1 in document.Children)
            {
                foreach (var item_2 in keylist)
                {
                    if (item_1.Char == item_2.star || item_1.Char == item_2.end)
                    {
                        token.Add((item_1, index));
                    }
                }
                index++;
            }
            //简单解析
            IDocumentElementList.Clear();
            Parse(token);
        }
        /// <summary>
        /// 计算需要折叠的行号
        /// </summary>
        public void ComputeLinenum() {
            //计算关键词开始所在行数
            //var StarList = IDocumentElementList.Select(data => data);
            IndexList.Clear();
            var i = 0;
            foreach (var item in document.Lines)
            {
                //从索引计算行数
                IDocumentElementList.Where((v) =>
                {
                    //加个判断把，目前只有花括号可折叠
                    if ((v.star is DocumentChar c) && c.Char == '{' &&
                    v.star_index >= item.Start && v.star_index <= item.Start + item.Count)
                    {
                        v.star_index = i;
                        IndexList.Add(i);
                        return true;
        ...
        {
            /// <summary>
            /// 解析token把它当作字符索引使用，计算行数把他当作行数使用
            /// </summary>
            public int star_index { set; get; }
            /// <summary>
            /// 解析token把它当作字符索引使用，计算行数把他当作行数使用
            /// </summary>
            public int end_index { set; get; }
            public IDocumentElement star { set; get; }
            public IDocumentElement end { set; get; }
        }
        //处理花括号对应
        public List<(char star, char end)> keylist = new List<(char star, char end)>() {
            {('{', '}')},
            {('(', ')')}
        };
        public List<Keystruct> IDocumentElementList = new List<Keystruct>();
        /// <summary>
        /// token解析对应
        /// </summary>
        /// <param name="TokenList"></param>
        public void Parse(List<(DocumentChar dchar, int index)> TokenList)
        {
            Stack<Keystruct> key = new Stack<Keystruct>();
            foreach (var item in TokenList)
            {
                foreach (var item1 in keylist)
                {
                    if (item.dchar.Char == item1.star)
                    {
                        Keystruct keystruct = new Keystruct();
                        keystruct.star = item.dchar;
                        keystruct.star_index = item.index;
                        key.Push(keystruct);
                    }
                    else if (item.dchar.Char == item1.end)
                    {
                        if (key.Count == 0)
                        {
                            continue;
                        }
                        var keystruct = key.Pop();
                        keystruct.end = item.dchar;
                        keystruct.end_index = item.index;
                        IDocumentElementList.Add(keystruct);
                    }
                }
            }
            ...
            this.Children.Add(new Grid
            {
                ...
                Children = {
                    new ScrollViewer
                    {
                        ...
                        Content = TextBoxView,
                        Bindings = { 
                            { nameof(ScrollViewer.HorizontalScrollBarVisibility), nameof(TextBox.HScrollBarVisibility), 2 },
                            { nameof(ScrollViewer.VerticalScrollBarVisibility), nameof(TextBox.VScrollBarVisibility), 2 }
                        },
                        Attacheds =
                        {
                            {
        ...
        protected override Size ArrangeOverride(in Size finalSize)
        {
            ...
            //添加完毕字符，在计算一边当前焦点矩形
            (TextBoxView as TextBoxViewEx).GetFocuLine();
        ...
        public virtual void Paste()
        {
            ...
            if (Clipboard.Contains(DataFormat.Html) && psw == 0)
            {
